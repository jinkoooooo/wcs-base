package operato.logis.ecs.base.wcs.dto;
 ...
 * [위치 정보 포함 로케이션 VO]
 *
 * WCS 로케이션 마스터(ExtTbInventoryLocation) 정보와 ECS 랙 마스터(tb_eq_rack_mst)의
 * 물리 좌표(row/bay/level)를 통합하여 관리하는 객체입니다.
 *
 * [JOIN 조건 업데이트]
 * tb_inventory_location.loc_id = tb_eq_rack_mst.id
 * AND tb_inventory_location.rack_eq_id = tb_eq_rack_mst.eq_id
 *
 * [핵심 용도]
...
public class LocWithPosition {
    ...
    public String toString() {
        return String.format("LocWithPosition{code=%s, pos=(R:%d, B:%d, L:%d)}",
                (loc != null ? loc.getLocId() : "N/A"), row, bay, level);
    }
