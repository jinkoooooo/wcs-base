...
public class CranePositionDto {
    ...
    /** 스태커크레인 고유 ID (tb_eq_crane_mst.id) */
    private String equipmentId;

    /** 설비 코드 (tb_eq_crane_mst.eq_id) */
    ...
    /**
     * 스태커 크레인 상태 (EcsDBConsts.EqCraneStatus)
     * 0: READY (대기)
     * 1: RESERVE (예약)
     * 2: RUN (이동중)
     * 5: EMR_STOP (비상정지)
    ...
    /** 자동 모드 여부 (tb_eq_crane_mst.auto_yn) — legend-spec 분기용 원시값 */
    private Boolean autoYn;

    /** 사용 여부 (tb_eq_crane_mst.use_yn) — DISABLED 판정 원시값 */
    private Boolean useYn;

    /**
     * 정규화된 스태커 크레인 상태 (EquipmentStateClassifier.CraneState.name()).
