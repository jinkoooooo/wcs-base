...
/**
 * 대시보드 팝업 제어 정보 응답 DTO
 *
 * [설계 의도]
 * 팝업이 열릴 때 1번의 API 호출로 해당 설비의 모든 제어 관련 상태를
 * 한 번에 반환한다. UI는 이 값을 보고 어떤 제어 버튼을 활성화할지 결정한다.
 *
 * 필드 null 정책:
 * - locId / eqGroupId / locXxx: RACK 타입일 때만 유효 (로케이션 매핑이 된 설비)
 * - eqUseYn: RACK 외 설비(CONVEYOR, SHUTTLE)에서만 유효
 */
@Data
@Builder
@NoArgsConstructor
...
public class DashboardControlInfo {
    ...
    /**
     * 현재 활성 오더 상태 코드 (WcsDomainEnums.ShuttleOrderStatus)
     * UI에서 버튼 활성화 조건 판단용:
    ...
    public static class InventoryItem {
        ...
        // ===== 신규: Stock 상세 (tb_inventory_stock) =====
        ...
        // ===== 신규: Location 메타 (tb_inventory_location) =====
