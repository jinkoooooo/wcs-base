...
public class RealTimeDataService extends AbstractQueryService {
    ...
    public List<TbEqCraneMst> findShuttlesByEqId(String eqId) {
        if (!StringUtils.hasText(eqId)) {
            return Collections.emptyList();
        }
    ...
    public List<TbEqCraneMst> findShuttlesByEqIdAndFloor(String eqId, int floor) {
        if (!StringUtils.hasText(eqId)) {
            return Collections.emptyList();
        }
    ...
    public List<TbWcsCraneOrder> findActiveOrders() {
        Query condition = OrmUtil.newConditionForExecution();
    ...
    public List<TbWcsCraneOrder> findErrorOrders() {
        Query condition = OrmUtil.newConditionForExecution();
    ...
    private List<TbEqCvMst> getConveyorsByEqIds(List<String> eqIds) {
        logger.info("getConveyorsByEqIds");
    ...
    private List<TbEqCraneMst> getShuttlesByEqIds(List<String> eqIds) {
        List<TbEqCraneMst> result = new ArrayList<>();
        for (String eqId : eqIds) {
    ...
    @Transactional(readOnly = true)
    public List<TbEqCraneMst> getShuttlesByGroup(String eqGroupId) {
    ...
    @Transactional(readOnly = true)
    public List<TbEqCraneMst> getShuttlesByGroupAndFloor(String eqGroupId, int floor) {
        List<TbEqMst> carEqList = findEqMstByGroupAndType(eqGroupId, EQ_TYPE_SHUTTLE_CAR);
        if (carEqList.isEmpty()) {
            return Collections.emptyList();
        }
    ...
    public Map getDashboardInitialData(String lcId, String pageId) {
        ...
        // 3. 대시보드 데이터 조회
        List<TbEqCraneMst> shuttles = getShuttlesByGroupAndFloor(eqGroupId, floorLevel);
