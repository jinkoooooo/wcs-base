...
/**
 * Crane WebSocket STOMP 설정
 * 실시간 설비 상태 및 작업 데이터 전송을 위한 WebSocket 구성
 */
@Configuration
@EnableWebSocketMessageBroker
public class CraneWebSocketConfig implements WebSocketMessageBrokerConfigurer {
    ...
    public void registerStompEndpoints(StompEndpointRegistry registry) {
        // WebSocket 연결 엔드포인트
        registry.addEndpoint("/ws/crane")
                .setAllowedOriginPatterns("*")
                .withSockJS();

        // SockJS 없는 순수 WebSocket 연결 (선택적)
        registry.addEndpoint("/ws/crane-raw")
                .setAllowedOriginPatterns("*");
    }
}
