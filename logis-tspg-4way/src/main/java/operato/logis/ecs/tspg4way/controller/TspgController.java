...
import operato.logis.ecs.tspg4way.core.CoreManager;
...
public class TspgController {
    ...
    @PostMapping("/shuttle/service/path-reset")
    public ResponseEntity<?>  shuttlePathReset(
            @RequestParam String shuttleCarId
    ){
        try {
            Tspg4WayShuttlePlc shuttle = tspgShuttlePlcRegistry.getEquipment(shuttleCarId);
            shuttle.getCar().initPathCmdList();
        }catch (Exception e){
            log.error(e.toString());
        }
        return new ResponseEntity<>(HttpStatus.OK);
