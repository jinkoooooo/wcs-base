package operato.logis.ecs.tspg4way.dashboard.realtime.scheduler;
...
import operato.logis.ecs.tspg4way.dashboard.realtime.model.BroadcastKey;
...
/**
 * 실시간 브로드캐스트 스케줄러. 첫 구독 시 enabled Provider 별 주기 태스크 시작, 마지막 해제 시 정리.
 * FetchContext/토픽 캐시 재사용 + payload 해시 변경 감지로 250ms 고주기 부하 완화.
 */
@Component
public class RealTimeBroadcastScheduler {
    ...
    public void init() {
        ...
        logger.info("[ Realtime ][ Broadcast ] initialized: poolSize={}", poolSize);
    }

    @PreDestroy
    public void shutdown() {
        logger.info("[ Realtime ][ Broadcast ] shutting down");
        stopAllBroadcasts();
        executor.shutdown();
        try {
    ...
    public synchronized void startBroadcast(String eqGroupId, String lcId, String pageId) {
        ...
        if (activeBroadcasts.containsKey(broadcastKey)) {
            ...
            logger.info("[ Realtime ][ Broadcast ] subscriber added: broadcastKey={}, subscribers={}",
        ...
        for (RealTimeDataProvider<?> provider : registry.getEnabledProviders()) {
            ...
            if (!runningTasks.containsKey(taskKey)) {
                ...
                logger.debug("[ Realtime ][ Broadcast ] task started: {}", taskKey);
            }
        }
        ...
        logger.info("[ Realtime ][ Broadcast ] started: broadcastKey={}, providers={}", broadcastKey, registry.getEnabledProviders().size());
    }
    ...
    public synchronized void stopBroadcast(String eqGroupId, String pageId) {
        ...
        if (count == null || count <= 1) {
            ...
            logger.info("[ Realtime ][ Broadcast ] subscriber removed: broadcastKey={}, remaining={}", broadcastKey, count - 1);
        }
    }
    ...
    private void doStopBroadcast(String broadcastKey) {
        ...
        runningTasks.entrySet().removeIf(entry -> {
            if (entry.getKey().endsWith(suffix)) {
                ...
                logger.debug("[ Realtime ][ Broadcast ] task stopped: {}", taskKey);
                return true;
            }
            return false;
        });
        ...
        logger.info("[ Realtime ][ Broadcast ] stopped: broadcastKey={}", broadcastKey);
    }
    ...
    public synchronized void stopAllBroadcasts() {
        ...
        contextCache.clear();
        logger.info("[ Realtime ][ Broadcast ] all stopped");
    }
    ...
    private void broadcastProviderData(RealTimeDataProvider<?> provider, String eqGroupId, String lcId, String pageId) {
        try {
            ...
            logger.debug("[ Realtime ][ Broadcast ] sent: count={}, type={}, topic={}", data.size(), provider.getProviderType(), topic);

        } catch (Exception e) {
            if (provider.retryOnError()) {
                logger.warn("[ Realtime ][ Broadcast ] error (retry): type={}", provider.getProviderType(), e);
            } else {
                logger.error("[ Realtime ][ Broadcast ] error (no retry): type={}", provider.getProviderType(), e);
            }
        }
    }
