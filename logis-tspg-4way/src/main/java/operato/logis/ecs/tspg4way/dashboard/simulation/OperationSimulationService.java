package operato.logis.ecs.tspg4way.dashboard.simulation;
...
public class OperationSimulationService extends AbstractQueryService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(OperationSimulationService.class);
    ...
    public Map<String, Object> seed(String eqGroupId, int floors, int shuttlesPerFloor,
        ...
        logger.info("[ Sim ][ Seed ] creating equipment: eqGroupId={}, floors={}, shuttlesPerFloor={}, rack={}x{}",
        ...
        logger.info("[ Sim ][ Seed ] Created: {} cars, {} rack cells, {} conveyors", carCount, rackCount, cvCount);
    ...
    private void ensureEqGroup(String eqGroupId) {
        ...
        if (count == null || count == 0) {
            ...
            logger.info("[ Sim ][ Seed ] Created eq_group: {}", eqGroupId);
        }
    }
    ...
    private void ensureEqMst(String id, String eqGroupId, String name, int type) {
        ...
        if (count == null || count == 0) {
            ...
            logger.info("[ Sim ][ Seed ] Created eq_mst: {} (type={})", id, type);
        }
    }
    ...
    private void ensureCarMst(String carId, String rackEqId, int level, int row, int bay) {
        ...
        if (count == null || count == 0) {
            ...
            logger.info("[ Sim ][ Seed ] Created car_mst: {} at F{}-R{}-B{}", carId, level, row, bay);
        } else {
            // 기존 데이터 초기화
            String updateSql = """
    ...
    private void clearSimulationOrders(String eqGroupId) {
        ...
        logger.info("[ Sim ][ Clear ] Deleted simulation orders");
    }
    ...
    public synchronized Map<String, Object> start(String eqGroupId) {
        ...
        logger.info("[ Sim ][ Run ] Started for eqGroupId: {}, rackEqId: {}", eqGroupId, rackEqId);
    ...
    public synchronized Map<String, Object> reset(String eqGroupId) {
        ...
        logger.info("[ Sim ][ Run ] Reset complete");
    ...
    private void initCarStates() {
        ...
        logger.info("[ Sim ][ Init ] Loaded {} shuttles", carStates.size());
    }
    ...
    private void tickShuttleMovement() {
        ...
        try {
            ...
            logger.error("[ Sim ][ Shuttle ] Movement tick error", e);
        }
    }
    ...
    private void tickOrderProcess() {
        ...
        try {
            ...
            logger.error("[ Sim ][ Order ] Process tick error", e);
        }
    }
    ...
    public void createNewOrder() {
        ...
        if (fromCell == null) {
            logger.debug("[ Sim ][ Order ] Nofrom cell found");
            return;
        }
        ...
        if (toCell == null) {
            logger.debug("[ Sim ][ Order ] Noto cell found");
            return;
        }
        ...
        logger.info("[ Sim ][ Order ] Created MOVE order: {} (F{} R{}B{} -> R{}B{})",
    ...
    private void processEcsRackOrders() {
        ...
        for (Map order : orders) {
            ...
            logger.debug("[ Sim ][ Ecs ] Assigned order {} to car {}", orderKey, availableCar.eqId);
        }
    }
    ...
    private void completeCarTask(SimCarState car) {
        ...
        if (car.row == fromRow && car.bay == fromBay && !car.hasCargo) {
            ...
            logger.debug("[ Sim ][ Car ] {} picked up cargo at R{}-B{}", car.eqId, car.row, car.bay);
        }
        // to 위치에 도착한 경우 -> 작업 완료
            ...
            logger.info("[ Sim ][ Car ] {} completed order {} at R{}-B{}", car.eqId, orderKey, car.row, car.bay);
        }
    }
