...
export function useFormRules(formData?: Recordable) {
  ...
  const getAccountFormRule = computed(() =>
    createRule(t("login.accountPlaceholder"))
  );
  const getPasswordFormRule = computed(() =>
    createRule(t("login.passwordPlaceholder"))
  );
  const getSmsFormRule = computed(() => createRule(t("login.smsPlaceholder")));
  const getMobileFormRule = computed(() =>
    createRule(t("login.mobilePlaceholder"))
  );

  const validatePolicy = async (_: RuleObject, value: boolean) => {
    return !value
      ? Promise.reject(t("login.policyPlaceholder"))
      : Promise.resolve();
  };
  ...
  const validateConfirmPassword = (password: string) => {
    return async (_: RuleObject, value: string) => {
      ...
      if (value !== password) {
        return Promise.reject(t("login.diffPwd"));
      }
