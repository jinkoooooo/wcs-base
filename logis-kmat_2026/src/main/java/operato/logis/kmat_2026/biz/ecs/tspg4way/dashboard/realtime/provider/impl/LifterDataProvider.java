...
import operato.logis.kmat_2026.biz.ecs.tspg4way.dashboard.realtime.dto.LifterStatusDto;
        ...
        try {
            ...
            String sql = """
                ...
                SELECT
                    ...
                    ar.from_loc_code        AS "currentFromLoc",
                    ar.to_loc_code          AS "currentToLoc"
            ...
            for (Map row : rows) {
                ...
                String toLocCode = toStringValue(getValue(row, "currentToLoc", "current_to_loc", "currenttoloc"));
                Integer targetLevel = parseFloorFromLocCodeOrNull(toLocCode);
                ...
                LifterStatusDto dto = LifterStatusDto.builder()
                        ...
                        .errorCode(toStringValue(getValue(row, "errorId", "error_id", "errorid")))
...
//            logger.info("result = {}",result);
    ...
    private Integer parseFloorFromLocCodeOrNull(String locCode) {
        if (!hasText(locCode)) return null;

        java.util.regex.Matcher matcher = java.util.regex.Pattern.compile("_F(\\d+)").matcher(locCode);
        if (matcher.find()) {
            try {
        ...
        matcher = java.util.regex.Pattern.compile("(\\d+)F").matcher(locCode);
        if (matcher.find()) {
            try {
                return Integer.parseInt(matcher.group(1));
            } catch (NumberFormatException ignored) {
