...
@Slf4j
public class TspgConveyorOrderService {

    private IQueryManager iQueryManager = BeanUtil.get(IQueryManager.class);
    private TspgConveyorPlcWriteService tspgConveyorPlcWriteService;
    private TspgConveyorPlcRegistry tspgConveyorPlcRegistry;
    private TspgConveyorPlc conveyorPlc;
    private String conveyorEqId;
    ...
    public TspgConveyorOrderService(TspgConveyorPlcWriteService tspgConveyorPlcWriteService, TspgConveyorPlcRegistry conveyorPlcRegistry, String conveyorEqId) {
        this.tspgConveyorPlcWriteService =  tspgConveyorPlcWriteService;
        this.tspgConveyorPlcRegistry = conveyorPlcRegistry;
        this.conveyorEqId = conveyorEqId;
        this.conveyorPlc = this.tspgConveyorPlcRegistry.getEquipment(this.conveyorEqId);
    }
    ...
    private void ConveyorOrderManager() {
        ...
        if(readyOrderList != null && !readyOrderList.isEmpty()) {
            ...
            List<TbEcsRouteOrder> outboundOrderList = readyOrderMap.get(EcsDBConsts.EcsRouteOrderCmdStatus.RACK_CV_READY.getValue());
    ...
    private void eqSendOrderManager(List<TbEcsRouteOrder> eqSendOrderList) {
        ...
        // 출고 지시인 경우 지시 출발지 랙단컨베이어 단과 리프트 컨베이어 단이 일치하면 지시 작업중
            ...
            rackCvOrderList.forEach(order -> {
                ...
                if(liftCvMst != null && rackCvMst != null && liftCvMst.getLevel() == rackCvMst.getLevel())
                    updateTbEcsRouteOrder(order, EcsDBConsts.OrderStatus.WORKING);
            });
        }
    }
    ...
    private void workingOrderManager(List<TbEcsRouteOrder> workingOrderList) {
        ...
        if(!workingOrderList.isEmpty()){
            workingOrderList.forEach(order -> {
                TbEqCvMst rackCvMst = selectTbEqCv(order.getToCvId());
                if(rackCvMst != null && rackCvMst.isCargoYn() && !rackCvMst.isRunYn())
                    if(order.getOrderType() == EcsDBConsts.OrderType.INBOUND.getValue()){
                        updateTbEcsRouteOrder(order, EcsDBConsts.OrderStatus.COMPLETE, EcsDBConsts.EcsRouteOrderCmdStatus.RACK_CV_READY);
                        // 랙단컨베이어 화물 예약 대기 업데이트
                        updateTbEqCvMst(rackCvMst, EcsDBConsts.EqConveyorStatus.READY);
                    }
    ...
    private void outboundOrderManager(TbEcsRouteOrder outboundOrder) {
        ...
        if(rackCv != null && liftCv != null
            ...
            if(outboundOrder.getOrderType() == EcsDBConsts.OrderType.MOVE.getValue()){
                ...
                command = conveyorPlc.getWirteMap().getMoveCommand(this.nextWorkId(), Integer.valueOf(outboundOrder.getFromCvId()), Integer.parseInt(outboundOrder.getToCvId()));

            }else{
                sendPlcMoveCommand();
                command = conveyorPlc.getWirteMap().getOutboundCommand(this.nextWorkId(), Integer.valueOf(outboundOrder.getFromCvId()), Integer.parseInt(outboundOrder.getToCvId()));
            }
    ...
    private void inboundOrderManager(TbEcsRouteOrder inboundOrder) {
        ...
        if(rackCv != null && inboundCv != null
            ...
            sendPlcMoveCommand();
            int[] command = conveyorPlc.getWirteMap().getOutboundCommand(this.nextWorkId(), Integer.parseInt(inboundOrder.getFromCvId()), Integer.parseInt(inboundOrder.getToCvId()));
    ...
    private void sendPlcCommand(int conveyorId, int[] command) {
        ...
        int firstDeviceCode = conveyorPlc.getWriteFirstDeviceCode(conveyorId);
        tspgConveyorPlcWriteService.sendCommandConveyor(this.conveyorEqId, MelsecConsts.DeviceCode.R, firstDeviceCode, command);
    }
