...
public class DmlJdbc2 extends AbstractDml implements Dml {
	...
	private Object toParamData(Object data) {
	    if (data instanceof Boolean)
			return queryMapper.convertBooleanValue((Boolean) data);
...
//			}
			try {
				subData = field.getType().getDeclaredConstructor().newInstance();
			} catch (InstantiationException e) {
				throw new DbistRuntimeException(e);
			} catch (IllegalAccessException e) {
				throw new DbistRuntimeException(e);
	...
	private Object appendSingleValueCondition(StringBuffer buf, String columnName, String paramKey, String operator, Character escape, Object value) {
		...
		if(value instanceof Boolean) {
	        // queryMapper는 DmlJdbc2나 상위 클래스에서 주입/참조 가능
			realValue = queryMapper.convertBooleanValue((Boolean) value);
		}
...
//					}
                ...
                // 실제 컬럼 정보를 가져옴
                List<TableColumn> columns = getTableColumnList(table);
                try {
                    ClassLoader contextLoader = Thread.currentThread().getContextClassLoader();

                    // 1) ByteBuddy를 사용해 동적 클래스 생성 빌더
                    ...
                    for (TableColumn col : columns) {
                        // dataType -> 자바 타입
                        Class<?> fieldType = toJavaClass(col.getDataType());
                        // 필드 이름: CamelCase
                        String fieldName = ValueUtils.toCamelCase(col.getName(), '_');
                    ...
                    Class<?> dynamicClass = builder
                        .make()
                        .load(contextLoader, ClassLoadingStrategy.Default.WRAPPER)
                        .getLoaded();

                    return dynamicClass;
                } catch (Exception ex) {
                    throw new DbistRuntimeException(ex);
                }
			}
		});
	}
	private static final Map<String, Class<?>> JAVA_CLASS_BY_DBDATATYPE_MAP;

	static {
	    JAVA_CLASS_BY_DBDATATYPE_MAP = new HashMap<>();

	    // 숫자 타입들은 BigDecimal로 매핑
	    JAVA_CLASS_BY_DBDATATYPE_MAP.put("number", BigDecimal.class);
	    JAVA_CLASS_BY_DBDATATYPE_MAP.put("int", BigDecimal.class);
	    JAVA_CLASS_BY_DBDATATYPE_MAP.put("bigint", BigDecimal.class);
	    JAVA_CLASS_BY_DBDATATYPE_MAP.put("smallint", BigDecimal.class);
	    JAVA_CLASS_BY_DBDATATYPE_MAP.put("tinyint", BigDecimal.class);
	    JAVA_CLASS_BY_DBDATATYPE_MAP.put("float", BigDecimal.class);
	    JAVA_CLASS_BY_DBDATATYPE_MAP.put("money", BigDecimal.class);
	    JAVA_CLASS_BY_DBDATATYPE_MAP.put("smallmoney", BigDecimal.class);
	    JAVA_CLASS_BY_DBDATATYPE_MAP.put("numeric", BigDecimal.class);
	    JAVA_CLASS_BY_DBDATATYPE_MAP.put("decimal", BigDecimal.class);

	    // 날짜/시간 타입들은 Date로 매핑
	    JAVA_CLASS_BY_DBDATATYPE_MAP.put("date", Date.class);
	    JAVA_CLASS_BY_DBDATATYPE_MAP.put("datetime", Date.class);
	    JAVA_CLASS_BY_DBDATATYPE_MAP.put("datetime2", Date.class);
	    JAVA_CLASS_BY_DBDATATYPE_MAP.put("smalldatetime", Date.class);
	    JAVA_CLASS_BY_DBDATATYPE_MAP.put("time", Date.class);
	    JAVA_CLASS_BY_DBDATATYPE_MAP.put("timestamp", Date.class);
	}

	private static Class<?> toJavaClass(String dbDataType) {
	    // dbDataType이 없거나 매핑되지 않았다면 기본적으로 String.class 반환
	    return JAVA_CLASS_BY_DBDATATYPE_MAP.getOrDefault(dbDataType, String.class);
	}
