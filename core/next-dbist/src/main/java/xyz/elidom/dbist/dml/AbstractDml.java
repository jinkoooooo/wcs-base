...
public abstract class AbstractDml implements Dml, ApplicationContextAware, BeanNameAware, InitializingBean {
	...
	@SuppressWarnings("unchecked")
	protected static <T> T newInstance(Class<T> clazz) throws InstantiationException, IllegalAccessException, IllegalArgumentException, InvocationTargetException, NoSuchMethodException, SecurityException {
		if (clazz.equals(Map.class)) {
			//return (T) ListOrderedMap.class.newInstance();  // Deprecated
			return (T) ListOrderedMap.class.getDeclaredConstructor().newInstance();
		}else {
			//return clazz.newInstance();//deprecated
	...
	private <T> T _insert(Class<T> clazz, Object data, String... fieldNames) throws Exception {
		...
		//T obj = ValueUtils.populate(data, clazz.newInstance()); //Deprecated
	    T obj = ValueUtils.populate(data, clazz.getDeclaredConstructor().newInstance());
	...
	protected ValueGenerator getValueGenerator(String generator) {
		...
		if (GenerationRule.UUID.equals(generator)) {
			try {
				return UuidGenerator.class.getDeclaredConstructor().newInstance();
		...
		try {
			//return (ValueGenerator) ClassUtils.getClass(generator).newInstance();//Deprecated
			Class<?> clazz = ClassUtils.getClass(generator); 
			return (ValueGenerator) clazz.getDeclaredConstructor().newInstance();
		} catch (InstantiationException e) {
			throw new DbistRuntimeException(e);
		} catch (IllegalAccessException e) {
			throw new DbistRuntimeException(e);
