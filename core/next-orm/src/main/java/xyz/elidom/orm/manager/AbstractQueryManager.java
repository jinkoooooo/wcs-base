...
public abstract class AbstractQueryManager implements IQueryManager {
	...
	public void upsert(Object data) {
		...
		try {
			isInsert = (this.isEmptyKeyValue(data) || this.getDml().select(data) == null);
		} catch (Exception e) {
			throw new ElidomDatabaseException(e);
		}
	...
	public void upsertBatch(List<?> list) {
		...
		try {
			this.beforeSaveList(list);

			for (Object data : list) {
				if (this.isEmptyKeyValue(data) || this.getDml().select(data) == null) {
					insertList.add(data);
				} else {
					updateList.add(data);
				}
	...
	public void upsertBatch(List<?> list, String... fieldNames) {
		...
		try {
			this.getDml().upsertBatch(list, fieldNames);
		} catch (Exception e) {
			throw new ElidomDatabaseException(e);
		}
	...
	public <T> T upsert(Class<T> clazz, Object data) {
		try {
			this.beforeSave(data);
			T upsertedOne = this.getDml().upsert(clazz, data);
			this.afterSave(upsertedOne);
			return upsertedOne;
	...
	public void upsertBatch(Class<?> clazz, List<?> list) {
		...
		try {
			this.getDml().upsertBatch(clazz, list);
		} catch (Exception e) {
			throw new ElidomDatabaseException(e);
		}
	...
	public void upsert(Class<?> clazz, Object data, String... fieldNames) {
		...
		try {
			this.getDml().upsert(clazz, data, fieldNames);
		} catch (Exception e) {
			throw new ElidomDatabaseException(e);
		}
	...
	public void upsertBatch(Class<?> clazz, List<?> list, String... fieldNames) {
		...
		try {
			this.getDml().upsertBatch(clazz, list, fieldNames);
		} catch (Exception e) {
			throw new ElidomDatabaseException(e);
		}
	...
	public void upsert(String tableName, Object data) {
		...
		try {
			this.getDml().upsert(tableName, data);
		} catch (Exception e) {
			throw new ElidomDatabaseException(e);
		}
	...
	public void upsertBatch(String tableName, List<?> list) {
		...
		try {
			this.getDml().upsertBatch(tableName, list);
		} catch (Exception e) {
			throw new ElidomDatabaseException(e);
		}
	...
	public void upsert(String tableName, Object data, String... fieldNames) {
		...
		try {
			this.getDml().upsert(tableName, data, fieldNames);
		} catch (Exception e) {
			throw new ElidomDatabaseException(e);
		}
	...
	public void upsertBatch(String tableName, List<?> list, String... fieldNames) {
		...
		try {
			this.getDml().upsertBatch(tableName, list, fieldNames);
		} catch (Exception e) {
			throw new ElidomDatabaseException(e);
		}
	 ...
	 * @param data
	 */
	private <T> void beforeCreate(T data) {
	 ...
	 * @param data
	 */
	private <T> void afterCreate(T data) {
		IEntityHook hook = this.checkEntityHook(data);
		if (hook != null) {
			hook.afterCreate();
		}
	 ...
	 * @param list
	 */
	private <T> void beforeCreateList(List<T> list) {
	 ...
	 * @param list
	 */
	private <T> void afterCreateList(List<T> list) {
		if (!ValueUtil.isEmpty(list)) {
			IEntityHook hook = this.checkEntityHook(list.get(0));
			if (hook != null) {
				for (T data : list) {
	 ...
	 * @param data
	 */
	private <T> void beforeUpdate(T data) {
		IEntityHook hook = this.checkEntityHook(data);
		if (hook != null) {
			hook.beforeUpdate();
		}
	 ...
	 * @param data
	 */
	private <T> void afterUpdate(T data) {
		IEntityHook hook = this.checkEntityHook(data);
		if (hook != null) {
			hook.afterUpdate();
		}
	 ...
	 * @param list
	 */
	private <T> void beforeUpdateList(List<T> list) {
		if (!ValueUtil.isEmpty(list)) {
			IEntityHook hook = this.checkEntityHook(list.get(0));
			if (hook != null) {
				for (T data : list) {
	 ...
	 * @param list
	 */
	private <T> void afterUpdateList(List<T> list) {
		if (!ValueUtil.isEmpty(list)) {
			IEntityHook hook = this.checkEntityHook(list.get(0));
			if (hook != null) {
				for (T data : list) {
	 ...
	 * 생성 혹은 업데이트 전 액션
	 */
	private <T> void beforeSave(T data) {
		IEntityHook hook = this.checkEntityHook(data);
		if (hook != null) {
			hook.beforeUpdate();
		}
	 ...
	 * 생성 혹은 업데이트 후 액션
	 */
	private <T> void afterSave(T data) {
		IEntityHook hook = this.checkEntityHook(data);
		if (hook != null) {
			hook.afterUpdate();
		}
	 ...
	 * @param list
	 */
	private <T> void beforeSaveList(List<T> list) {
		if (!ValueUtil.isEmpty(list)) {
			IEntityHook hook = this.checkEntityHook(list.get(0));
			if (hook != null) {
				for (T data : list) {
	 ...
	 * @param list
	 */
	private <T> void afterSaveList(List<T> list) {
		if (!ValueUtil.isEmpty(list)) {
			IEntityHook hook = this.checkEntityHook(list.get(0));
			if (hook != null) {
				for (T data : list) {
	 ...
	 * 삭제 전 액션
	 */
	private <T> void beforeDelete(T data) {
		IEntityHook hook = this.checkEntityHook(data);
		if (hook != null) {
			hook.beforeDelete();
		}
	 ...
	 * 삭제 후 액션
	 */
	private <T> void afterDelete(T data) {
		IEntityHook hook = this.checkEntityHook(data);
		if (hook != null) {
			hook.afterDelete();
		}
	 ...
	 * @param list
	 */
	private <T> void beforeDeleteList(List<T> list) {
		if (!ValueUtil.isEmpty(list)) {
			IEntityHook hook = this.checkEntityHook(list.get(0));
			if (hook != null) {
				for (T data : list) {
	 ...
	 * @param list
	 */
	private <T> void afterDeleteList(List<T> list) {
		if (!ValueUtil.isEmpty(list)) {
			IEntityHook hook = this.checkEntityHook(list.get(0));
			if (hook != null) {
				for (T data : list) {
	 ...
	 * @param data
	 */
	private <T> void beforeFind(T data) {
		IEntityHook hook = this.checkEntityHook(data);
		if (hook != null) {
			hook.beforeFind();
		}
	 ...
	 * 조회(Sinlge) 후 액션
	 */
	private <T> void afterFind(T data) {
		IEntityHook hook = this.checkEntityHook(data);
		if (hook != null) {
			hook.afterFind();
		}
	 ...
	 * @param data
	 */
	private <T> void beforeSearch(T data) {
		IEntityHook hook = this.checkEntityHook(data);
		if (hook != null) {
			hook.beforeSearch();
		}
	 ...
	 * 조회(Multi) 후 액션
	 */
	private <T> void afterSearch(List<T> datas) {
	...
	private <T> IEntityHook checkEntityHook(T data) {
		return (data instanceof IEntityHook) ? (IEntityHook) data : null;
	}

	private boolean isEmptyKeyValue(Object data) {
		Query query = null;
		try {
