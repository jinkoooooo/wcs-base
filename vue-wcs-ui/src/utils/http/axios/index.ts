...
/**
 * prefix와 url 사이의 / 중복을 정리해서 안전하게 URL을 조합한다.
 * 예)
 * /rest + /terminologies       => /rest/terminologies
 * /rest/ + /terminologies      => /rest/terminologies
 * /rest + //terminologies      => /rest/terminologies
 */
const joinUrl = (prefix = '', url = '') => {
...
const transform: AxiosTransform = {
  ...
  transformResponseHook: (res: AxiosResponse<Result>, options: RequestOptions) => {
    ...
    // // 여기서는 자신의 프로젝트의 실제 상황에 따라 다른 code에 대해 다른 작업을 수행합니다.
    // // 현재 요청의 중단을 원치 않으시면 데이터를 return해주시고, 그렇지 않으면 바로 이상을 던져주시면 됩니다
    // let timeoutMsg = '';
    // switch (code) {
    ...
    //     }
    // }
    // // errorMessageMode='modal'일 때 메시지 프롬프트가 아닌 modal 오류 팝업이 표시되므로 중요한 오류에 사용됩니다.
    // // errorMessageMode='none' 은 일반적으로 호출할 때 오류 메시지가 자동으로 뜨지 않도록 명시되어 있습니다.
  ...
  beforeRequestHook: (config, options) => {
    ...
    if (joinPrefix) {
      config.url = joinUrl(urlPrefix, config.url);
    }

    if (apiUrl && isString(apiUrl)) {
      config.url = joinUrl(apiUrl, config.url);
    }
    ...
    try {
      ...
      if (errMessage) {
        if (errorMessageMode === 'modal') {
          createErrorModal({ title: errCd, content: errMessage });
