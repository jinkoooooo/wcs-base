  ...
  return searchMetaList.map(function (metaData) {
    ...
    if (metaData.search_init_val) {
      if (metaData.search_editor == 'date-picker') {
        ...
        field.defaultValue = [
          dayjs(calcDate(metaData.search_init_val), 'YYYY-MM-DD').subtract(5, 'day'),
 ...
 * 기존 파싱 로직 참조
 */
// function (field, metaData, resourceForm) {
//   let editor = metaData.search_editor;
...
//     if (editor.indexOf('.') > 0) {
//       let editorSplitArr = editor.split('.');
...
//     let refName =
//       field.name.indexOf('_id') > 0 ? field.name.substr(0, field.name.indexOf('_id')) : field.name;
//     field.userData = {
...
//     parseSearchValueRange(field, metaData.range_val, 'number');
//     //parseSearchValueRange(field, metaData.search_init_val, 'number');
...
//     parseSearchValueRange(field, metaData.search_init_val, 'date');
//     //parseSearchValueRange(field, metaData.range_val, 'date');
...
//       if (metaData.search_init_val) field.userData.defValue = calcDate(metaData.search_init_val);
//     }
//   } else if (
//     'time-picker' == editor ||
//     'datetime-picker' == editor ||
//     'ranged-datetime-picker' == editor
//   ) {
...
//       parseDatetimeInitValue(field, metaData.search_init_val);
//     }
//   }
// };
