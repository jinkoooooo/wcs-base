...
export async function useAutoFocus({
  ...
  watchEffect(async () => {
    //console.log(unref(getProps).autoFocusItemName);
    if (unref(isInitedDefault) || !unref(getProps).autoFocusItemName) {
      return;
    }
    await nextTick();
    const schemas = unref(getSchema);
    ...
    const autoFocusName = 'form_item_' + props.autoFocusItemName;
    const el = (formEl as any)?.$el as HTMLElement;
    if (!formEl || !el || !schemas || schemas.length === 0) {
      return;
    }
    ...
    const inputEl = el.querySelector(`#${autoFocusName}`) as Nullable<HTMLInputElement>;
    if (!inputEl) return;
    inputEl?.focus();
  });
}
