<template>
  ...
  <Card
    ...
    :style="styleRef"
  >
  <div class="content" :style="contentLocaleStyleRef">
    <!-- CHART AREA -->
      <div class="box">
        <div ref="chartRef" :style="chartLocaleStyleRef" ></div>
      </div>

    <!-- DETAIL DATA AREA -->
      <div class="box" :style="boxLocaleStyleRef">
        <div>
          <div style="font-weight: bold;" :style="titleLocaleStyleRef">{{ t('label.summary') }}</div>
        ...
        <div >
          <div :style="titleLocaleStyleRef" style="margin-top:0.2rem;">{{ t('label.detail_information') }}</div>
...
/************* 스타일 *************/
const contentLocaleStyleRef =computed(()=>{
  return {marginTop:locale.value==koLocaleRef.value ? null : '0.5rem'}
})

const chartLocaleStyleRef = computed(() => ({
  height: chartHeight.value
}));

const boxLocaleStyleRef = computed(() => (
  { marginLeft:isKorean.value ? '1rem' : '0.2rem'}
));

const chartHeight=ref('6rem');

const styleRef = ref({border: '1px solid #00BFFF',borderRadius:'0px',backgroundColor:'#00386C',height:'9.85rem'})

const titleLocaleStyleRef =computed(()=>{
  return {fontSize:isKorean.value ? '0.7rem' : '0.5rem',
          fontWeight: 'bold',}
})

const localeStyleRef =computed(()=>{
  return {fontSize:isKorean.value ? '0.6rem':'0.45rem'}
})
...
const titleRef = ref('SW Inverter');
