<template>
  ...
  >
    ...
    <ForceCompletionPopup @register="registerForceCompletionModal" :result="resultForceRef" width="fit-content" />
  </CommonPage>
</template>
...
import ForceCompletionPopup from './popup/ForceCompletionPopup.vue';
...
const resultForceRef = ref();
const [registerForceCompletionModal, { openModal: openForceCompletionPopup }] = useModal();
...
const forceCompletion = () => {
  ...
  if (selectedRows.length > 1)
    return notification.error({
      message: '에러',
      description: `하나의 작업만 선택 가능합니다.`,
      duration: 1,
    });
  if (selectedRows[0].process_status > 32)
    return notification.error({
      message: '에러',
      description: `이미 처리된 작업은 선택할 수 없습니다.`,
      duration: 1,
    });

  openForceCompletionPopup(true, selectedRows[0]);
};
