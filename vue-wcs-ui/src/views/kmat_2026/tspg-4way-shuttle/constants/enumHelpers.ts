...
export interface EnumEntry<TCode = string | number> {
  ...
  badgeVariant?: string;
  /** 헥사 색상 (#RRGGBB) — legend / 틴트 / 배지 색 단일 출처 */
  color?: string;
  /** 틴트 반투명도 (0~1) — 0이면 틴트 없음 */
  tintAlpha?: number;
}
...
/** color lookup */
export function enumColor<TCode>(
  enumDef: Record<string, EnumEntry<TCode>>,
  code: TCode | null | undefined,
  fallback = '',
): string {
  return findEnumEntry(enumDef, code)?.color ?? fallback;
}

/**
 * EnumEntry 객체 → select 옵션 배열 변환 (라벨/코드 모두 한 정의에서 가져옴)
 *
 * @example
 *   enumToSelectOptions(WcsOrderStatus)
 *   // → [{ value: 0, label: '생성됨 (0)' }, ...]
 */
export function enumToSelectOptions<TCode extends string | number>(
  enumDef: Record<string, EnumEntry<TCode>>,
): { value: TCode; label: string }[] {
  return Object.values(enumDef).map((entry) => ({
    value: entry.code,
