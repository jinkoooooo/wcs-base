<template>
  ...
  >
    <template #customDesc>
      <div class="rounded-md border border-slate-200 bg-white shadow-sm mb-2">
        <div class="px-4 py-3">
          <div class="text-sm font-semibold text-slate-800">{{ t("text.center_management_desc")}}</div>
        </div>
      </div>
    </template>
  ...
  const deleteRows = () => {
    ...
    createConfirm({
      ...
      onOk: async () => {
        ...
        console.log('selectedActualRows =', selectedActualRows);
        console.log('getCheckedRows =', gridRef.value.getCheckedRows());
